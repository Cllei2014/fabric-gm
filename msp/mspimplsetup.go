...
func (msp *bccspmsp) getCertifiersIdentifier(certRaw []byte) ([]byte, error) {
	...
	var chain []*x509GM.Certificate
	if root {
		chain = []*x509GM.Certificate{cert}
	} else {
...
func (msp *bccspmsp) setupCAs(conf *m.FabricMSPConfig) error {
	...
	msp.opts = &x509GM.VerifyOptions{Roots: x509GM.NewCertPool(), Intermediates: x509GM.NewCertPool()}
...
func (msp *bccspmsp) setupTLSCAs(conf *m.FabricMSPConfig) error {
	...
	opts := &x509GM.VerifyOptions{Roots: x509GM.NewCertPool(), Intermediates: x509GM.NewCertPool()}

	// Load TLS root and intermediate CA identities
	msp.tlsRootCerts = make([][]byte, len(conf.TlsRootCerts))
	rootCerts := make([]*x509GM.Certificate, len(conf.TlsRootCerts))
	...
	intermediateCerts := make([]*x509GM.Certificate, len(conf.TlsIntermediateCerts))
	...
	// ensure that our CAs are properly formed and that they are valid
	for _, cert := range append(append([]*x509GM.Certificate{}, rootCerts...), intermediateCerts...) {
		if cert == nil {
			continue
		}
